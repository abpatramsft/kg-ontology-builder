"""
Deploy the Seattle Hotel Agent to Azure AI Foundry as a hosted agent.

Prerequisites:
  Build & push the image manually first (from the poc/ directory):

    az acr build --registry aicouncilacr285 --image seattle-hotel-agent:<TAG> --platform linux/amd64 .
...
IMAGE_NAME = "seattle-hotel-agent"                 # Repository name inside ACR
IMAGE_TAG = "v1"                                   # Set this to the tag you used in az acr build
AGENT_NAME = "seattle-hotel-agent"                 # Agent name in Foundry
