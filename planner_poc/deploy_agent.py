"""
Deploy the Planner Agent to Azure AI Foundry as a hosted agent.

Prerequisites:
  Build & push the image manually first (from the planner_poc/ directory):
...
IMAGE_NAME = "planner-agent"                       # Repository name inside ACR
IMAGE_TAG = "v1"                                   # Set this to the tag you used in az acr build
AGENT_NAME = "planner-agent"                       # Agent name in Foundry
...
def deploy_agent():
    ...
    cosmos_key = COSMOS_DB_KEY or os.getenv("COSMOS_DB_KEY", "")
    if not cosmos_key:
        print("WARNING: COSMOS_DB_KEY is empty. Set it in the script or as an environment variable.")
